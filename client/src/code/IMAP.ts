...
export class Worker {
  ...
  public async listMessages(inMailbox: string): Promise<IMessage[]> {
    ...
    const encodedMailbox = encodeURIComponent(inMailbox);

    const response: AxiosResponse = await axios.get(`${config.serverAddress}/mailboxes/${encodedMailbox}`);
    return response.data;
  ...
  public async getMessageBody(inID: string, inMailbox: string): Promise<string> {
    ...
    // encode inMailbox and inID
    const encodedMailbox = encodeURIComponent(inMailbox);
    const encodedID = encodeURIComponent(inID);
  
    const response: AxiosResponse = await axios.get(`${config.serverAddress}/messages/${encodedMailbox}/${encodedID}`);
    return response.data;
